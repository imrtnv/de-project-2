...
select distinct (regexp_split_to_array(s.vendor_agreement_description,E'\\:'))[1]::bigint as agreementid,
...
select distinct shippingid, 
	...
	(regexp_split_to_array(s.vendor_agreement_description,E'\\:'))[1]::bigint
from shipping s
...
with cte as (select shippingid, max(case when s.state ='booked' then s.state_datetime end) shipping_start_fact_datetime,
	max(case when s.state ='recieved' then s.state_datetime end) shipping_end_fact_datetime,
	max(s.state_datetime) as max_datetime
from shipping s 
group by 1)

select s.shippingid, s.status, s.state, c.shipping_start_fact_datetime, c.shipping_end_fact_datetime
from public.shipping s
inner join cte c
on s.shippingid=c.shippingid
and s.state_datetime = c.max_datetime
